...
Eigen::Affine3d webotsNodeToAffine3d(webots::Node *node)
{
  ...
  Eigen::Affine3d T = (Eigen::Affine3d)Eigen::AngleAxisd(
    angle_axis[3], Eigen::Vector3d(angle_axis[0], angle_axis[1], angle_axis[2]));
...
CommNavigationObjects::BoundingBox webotsExtentsToBoundingBox(
  ...
  CommNavigationObjects::BoundingBox box;
  auto position = node->getPosition();
  auto rotation_matrix = node->getOrientation();
  Eigen::Matrix3d m;
  m << rotation_matrix[0], rotation_matrix[1], rotation_matrix[2],
      rotation_matrix[3], rotation_matrix[4], rotation_matrix[5],
      rotation_matrix[6], rotation_matrix[7], rotation_matrix[8];
  min = m * min + Eigen::Vector3d({position[0], position[1], position[2]});
  max = m * max + Eigen::Vector3d({position[0], position[1], position[2]});
  box.setXmin(min.x());
  box.setYmin(min.y());
  box.setZmin(min.z());
  box.setXmax(max.x());
...
CommNavigationObjects::BoundingBox webotsNodeSize2DToBoundingBox(
  webots::Node *node)
{
  auto size = node->getField("size")->getSFVec2f(); 
  Eigen::Vector3d pos_min = {-size[0]/2, 0, -size[1]/2};
  Eigen::Vector3d pos_max = {size[0]/2, 0, size[1]/2};
  return webotsExtentsToBoundingBox(pos_min, pos_max, node);
}

CommNavigationObjects::BoundingBox webotsNodeSize3DToBoundingBox(
  webots::Node *node)
{
  auto size = node->getField("size")->getSFVec3f(); 
  Eigen::Vector3d pos_min = {-size[0]/2, 0, -size[2]/2};
  Eigen::Vector3d pos_max = {size[0]/2, size[1], size[2]/2};
  return webotsExtentsToBoundingBox(pos_min, pos_max, node);
}

CommNavigationObjects::BoundingBox webotsNodeCylinderToBoundingBox(
  webots::Node* node) {
  auto height = node->getField("height")->getSFFloat();
  auto radius = node->getField("radius")->getSFFloat();
  Eigen::Vector3d pos_min = {-radius, 0, -radius};
  Eigen::Vector3d pos_max = {radius, height, radius};
  return webotsExtentsToBoundingBox(pos_min, pos_max, node);;
}

CommNavigationObjects::BoundingBox webotsPalletToBoundingBox(
    webots::Node *node) {
  auto size = node->getField("palletSize")->getSFVec3f(); 
  auto number = node->getField("palletNumber")->getSFInt32();
  Eigen::Vector3d pos_min = {-size[0]/2, 0, -size[2]/2};
  Eigen::Vector3d pos_max = {size[0]/2, 2 * number * size[1]/2, size[2]/2};
  return webotsExtentsToBoundingBox(pos_min, pos_max, node);
}

CommNavigationObjects::BoundingBox webotsCabinetToBoundingBox(
    webots::Node *node) {
  auto depth = node->getField("depth")->getSFFloat(); 
  auto heights = node->getField("rowsHeights");

  double h;
  for (size_t i = 0; i < heights->getCount(); ++i)
    h += heights->getMFFloat(i);
  
  auto widths = node->getField("columnsWidths");
  double w;
  for (size_t i = 0; i < widths->getCount(); ++i)
    w += widths->getMFFloat(i);

  Eigen::Vector3d pos_min = {-w/2, 0, 0};
  Eigen::Vector3d pos_max = {w/2, h, depth};
  return webotsExtentsToBoundingBox(pos_min, pos_max, node);
}
